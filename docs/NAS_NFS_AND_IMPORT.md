...
```bash
mkdir -p /mnt/nas-user-a /mnt/nas-user-b
mount -a
...
```bash
export LEGACY_NAS_FINANZEN=/mnt/nas-user-a   # nach Export + mount -a
export LEGACY_MIGRATE_STATE_DIR=/mnt/paperless-data/migrate-user-a
...
Monika PDF analog mit `migrate-user-b-pdf`, `consume/monika-inbox`, optional `EXCLUDE_REGEX` für Foto/Craft-Ordner.
